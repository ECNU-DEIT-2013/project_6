...
var server = await HttpServer.bind(InternetAddress.LOOPBACK_IP_V4, 8015);
  ...
  await for (HttpRequest request in server) {
    ...
    /*else if (request.uri.path == "/temp.json") {
      print("read json");
      handleGet(request);
      routercheck.route(request);
    }
...
check(HttpRequest request) async{
  var a;
  List list = [];
  var s = JSON.decode(jsondata);
  var name = s[0];
 ...
 // Map<String,String> rs={'check':'ok'};
  //request.session.addAll(rs);
  }
 // HttpSession.addAll('CHECK','OK');
