...
1、application package:
    application package不是作为library package来使用，application package可能依赖其他的packages，但是不依赖自己；
    library package既依赖其他packages,又依赖自己。Application packages可以在命令行或浏览器中直接运行。
    application package需要检查他们的lockfiles（lock文件）以保持依赖的一致性。
 ...
 2、asset:
  ...
  4、 同时，我们小组在交流翻译内容，讨论阅读感受的时候，我从中知道：
     transformer可以把dart或html文件进行相应的转化；
     在使用package时，是有着传递依赖性的；
     在source中可以获取package的下载路径；
     entrypoint通常是一个dart文件（例如在bin文件夹里的dart文件）；
   5、不懂的概念：
    pub build 和pub serve 的作用是什么？
    解释SDK constraint的那一段其实没有懂
